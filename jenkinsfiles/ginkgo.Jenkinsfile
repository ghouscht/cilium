    ...
    stages {
        ...
        stage ("BDD-Test-PR"){
            ...
            environment {
                ...
                CILIUM_IMAGE = """${sh(
                        returnStdout: true,
                        script: 'echo -n $(${GOPATH}/${PROJ_PATH}/test/print-node-ip.sh)/cilium/cilium'
                        )}"""
                CILIUM_TAG = "latest"
                CILIUM_OPERATOR_IMAGE= """${sh(
                        returnStdout: true,
                        script: 'echo -n $(${GOPATH}/${PROJ_PATH}/test/print-node-ip.sh)/cilium/operator'
                        )}"""
                CILIUM_OPERATOR_TAG = "latest"
                HUBBLE_RELAY_IMAGE= """${sh(
                        returnStdout: true,
                        script: 'echo -n $(${GOPATH}/${PROJ_PATH}/test/print-node-ip.sh)/cilium/hubble-relay'
                        )}"""
